...
st.title("Top 3 Heart Disease Predictors (Cached Models)")
...
@st.cache_resource
def train_and_cache_models(file):
    df = pd.read_csv(file)
...
if uploaded_file:
    top3, numeric_features, categorical_features, X_example = train_and_cache_models(uploaded_file)
